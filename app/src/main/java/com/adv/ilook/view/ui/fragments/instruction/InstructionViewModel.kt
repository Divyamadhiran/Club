...
constructor(
    ...
    private val _instructionScrRemoteFlow=MutableLiveData<Map<Int,Boolean>>()
    val instructionScrRemoteFlow:LiveData<Map<Int,Boolean>> get()=_instructionScrRemoteFlow
    ...
    override suspend fun init(function: (TypeOfData) -> Unit) {
       viewModelScope.launch {
           ...
           launch(Dispatchers.Main) {
               ...
               fetchInstructScrRemoteFlow()
            }
        }
    }


    private fun fetchInstructScrRemoteFlow() {
        val filedsCondition= mutableMapOf<Int,Boolean>()
        filedsCondition[R.id.img_back_arrow] to instructionScreen.views?.imageView?.icon?.enable
        filedsCondition[R.id.header_title] to instructionScreen.views?.textView?.termsOfUseText?.enable
        filedsCondition[R.id.webView] to instructionScreen.views?.webView?.enable
        filedsCondition[R.id.agree_button] to instructionScreen.views?.buttonView?.agreeBtn?.enable
        filedsCondition[R.id.disagree_button] to instructionScreen.views?.buttonView?.disagreeBtn?.enable
        _instructionScrRemoteFlow.postValue(filedsCondition)
    }
