...
class OtpViewModel @Inject constructor(
    ...
    private fun updateViewElements() {
        otpScreen.views?.apply {
            ...
            postIfEnabledAndNotEmpty(textView?.header?.enable,textView?.header?.helperText) {
                _tv_otp_helper_text.postValue(it)
            }

            postIfEnabledAndNotEmpty(textView?.otpCode?.enable,textView?.otpCode?.text) {
                _et_otp_number_text.postValue(it)
            }

            postIfEnabledAndNotEmpty(buttonView?.verifyOtp?.enable,buttonView?.verifyOtp?.text) {
            ...
            postIfEnabledAndNotEmpty(toastView?.loginSuccess?.enable,toastView?.loginSuccess?.text) {
                _toast_success_message.postValue(it)
            }
            postIfEnabledAndNotEmpty(toastView?.loginFailure?.enable,toastView?.loginFailure?.text) {
