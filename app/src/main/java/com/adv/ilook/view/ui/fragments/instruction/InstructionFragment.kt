...
class InstructionFragment() : BaseFragment<FragmentInstructionBinding>() {
    ...
    fun webViewSetUp(enable: Boolean = false, htmlStr: String = "app/src/main/assets/terms.html") {
        viewBinding?.apply {
            ...
            termsOfUseWebView.settings.javaScriptEnabled = false
            termsOfUseWebView.webViewClient = WebViewClient()
            if (enable)
                termsOfUseWebView.loadData(htmlStr, "text/html", "UTF-8")
            else
                termsOfUseWebView.loadUrl(htmlStr)
        }
    }
