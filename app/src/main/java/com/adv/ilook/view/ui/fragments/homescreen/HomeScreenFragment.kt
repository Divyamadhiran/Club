...
class HomeScreenFragment : BaseFragment<FragmentHomeScreenBinding>(),
    ...
    override fun setup(savedInstanceState: Bundle?) {
        ...
        viewModel.homeFieldsEnableFlow.observe(viewLifecycleOwner){datas->
            Log.d(TAG, "setup: homeFieldsControls $datas")
            homeFieldsControls(datas)
        }
    }
    ...
    private fun homeFieldsControls(datas: Map<Int, Boolean>?) {
        viewBinding?.apply {
            datas?.forEach{(id,value)->
                navigationViewStart?.menu?.findItem(id)?.isVisible = value
                Log.d("_TAG_", "homeFieldsControls: $datas")
            }
        }
    }
