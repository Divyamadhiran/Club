...
abstract class BaseFireStore(val db: FirebaseFirestore) {
    ...
    //Ref:firebase ex github

    internal fun runAll(function: (jsonStr: String,docId:Any) -> Unit) {
      ...
      // exampleData()
       // exampleDataCollectionGroup()
        ...
        //  addAdaLovelace()
        // addAlanTuring()
        getAvailableRooms(){ jsonStr:String,docId:Any ->
            function(jsonStr,docId)
        }
//        listenForUsers()
//        docReference()
//        collectionReference()
...
//        collectionGroupQuery()
    ...
    private fun getAvailableRooms(function: (jsonStr: String,docId:Any) -> Unit) {
        // [START get_all_users]
        db.collection("gen_data")
            .document("list_of_rooms")
            .get()
            .addOnSuccessListener { result ->
               ...
               /* for (document in result) {
                        ...
                        Log.d(TAG, "getAvailableRooms:id =${document.id} => data =${document.data}")
                        str = gson.toJson(document.data, gsonType)
                        function(str,document.id)


                }*/
                Log.d(TAG, "getAvailableRooms:id =${result.id} => data =${result.data}")
            ...
            .addOnFailureListener { exception ->
                ...
                exception.message?.let { function(it,"") }
            }
        // [END get_all_users]
    }
    ...
    private fun addDocument() {
        ...
        db.collection("cities")
            .add(data)
            .addOnSuccessListener { documentReference ->
                Log.d(TAG, "addDocument:DocumentSnapshot written with ID: ${documentReference.id}")
            }
            .addOnFailureListener { e ->
                Log.w(TAG, "addDocument: Error adding document", e)
            }
        // [END add_document]
    }
    ...
    fun writeBatch() {
        ...
        db.runBatch { batch ->
            ...
            // ...
            Log.d(TAG, "writeBatch: addOnCompleteListener")
        }
        // [END write_batch]
    }
    ...
    private fun exampleData() {
        ...
        val map1 = hashMapOf(
            ...
            "isAdded" to false,
            "room_icon" to "hall",
            "icon_select_color" to "green",
            "icon_deselect_color" to "red",
            "status" to listOf("", "")
        )
        ...
        val map2 = hashMapOf(
            "id" to "2",
            "room_name" to "Kitchen",
            "hardware_id" to "222",
            "favorite" to false,
            "roomCount" to 0,
            "isAdded" to false,
            "room_icon" to "kitchen",
            "icon_select_color" to "green",
            "icon_deselect_color" to "red",
            "status" to listOf("", "")
        )
        ...
        val map3 = hashMapOf(
            "id" to "3",
            "room_name" to "BedRoom",
            "hardware_id" to "333",
            "favorite" to false,
            "roomCount" to 0,
            "isAdded" to false,
            "room_icon" to "bedroom",
            "icon_select_color" to "green",
            "icon_deselect_color" to "red",
            "status" to listOf("", "")
        )
        ...
        val map4 = hashMapOf(
            "id" to "4",
            "room_name" to "Parking",
            "hardware_id" to "444",
            "favorite" to false,
            "roomCount" to 0,
            "isAdded" to false,
            "room_icon" to "parking",
            "icon_select_color" to "green",
            "icon_deselect_color" to "red",
            "status" to listOf("", "")
        )
        ...
        val map = hashMapOf("rooms" to list)
        rooms.document("list_of_rooms").set(map)
        // [END example_data]
    }

/*    open suspend fun writeSelectedRoomsDatas(docPath: String ,
                                            collectPath: String,
                                            keys: String, arrayList: ArrayList<RoomSelected?>, function: (res: Any) -> Unit){
        Log.d(TAG, "writeSelectedRoomsData: BaseClass")
    }*/
